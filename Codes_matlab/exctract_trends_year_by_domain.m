...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc; clear all ;
main_dir='G:\PROJETS\PROJET_CORDEX\CORDEX-NAM44\TENDANCES_ANNEES\';
out_dir='G:\PROJETS\PROJET_2018\TENDANCES_RCMs\';
     ...
     station=list_station(d);
       if strcmpi(domaine,'Domaine1')
        ...
        elseif strcmpi(domaine,'Domaine3')
         ...
         elseif strcmpi(domaine,'Domaine4')
          ...
          scen=scenario(1);
              ...
              elseif strcmpi(scen,'rcp85')   
                  PATHM={'CANRCM4_CanESM2_rcp85', ...           
                         'CRCM5-v1_CCCma-CanESM2_rcp85', ...   
                  ...
                  MODEL_LIST={'CANRCM4_NAM-44_ll_CanESM2_rcp85', ...           
                         'CRCM5-v1_NAM-44_ll_CCCma-CanESM2_rcp85', ...  
        ...
        for mo = 1:mm   %boucle sur les modeles
            ...
            FichierIn1 = char ( strcat(main_dir,curr_path,'\',char(var1),'\TENDANCES_SEN_',curr_model,'_',char(var1),'_',char(period1),'.nc') );  
                [InArr1, LatArr, LonArr] = read_netcdf(FichierIn1); 
           
            FichierIn2 = char ( strcat(main_dir,curr_path,'\',char(var1),'\TENDANCES_SEN_',curr_model,'_',char(var1),'_',char(period2),'.nc') );  
                [InArr2, LatArr, LonArr] = read_netcdf(FichierIn2);   
       
            FichierIn3 = char ( strcat(main_dir,curr_path,'\',char(var1),'\TENDANCES_SEN_',curr_model,'_',char(var1),'_',char(period3),'.nc') );  
        ...
        for j=1:(mm)
         ...
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_RCP45_',char(var1),'_',char(period1),'.csv'),rcp45_fin1)
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_RCP45_',char(var1),'_',char(period2),'.csv'),rcp45_fin2)
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_RCP45_',char(var1),'_',char(period3),'.csv'),rcp45_fin3)
        ...
        for j=1:(mm)
         ...
         for l=1:mm
            eval(['mat_1',num2str(l),'=[mat1',num2str(l),';NaN(max_mat1-length(mat1',num2str(l),'),1)];'])
            eval(['rcp85_fin1(:,',num2str(l),')=mat_1',num2str(l),'(1:end-1,:);'])
            eval(['mat_2',num2str(l),'=[mat2',num2str(l),';NaN(max_mat2-length(mat2',num2str(l),'),1)];'])
            eval(['rcp85_fin2(:,',num2str(l),')=mat_2',num2str(l),'(1:end-1,:);'])
            eval(['mat_3',num2str(l),'=[mat3',num2str(l),';NaN(max_mat3-length(mat3',num2str(l),'),1)];'])
            eval(['rcp85_fin3(:,',num2str(l),')=mat_3',num2str(l),'(1:end-1,:);'])
         end
         for l=1:mm
            eval(['clear mat1',num2str(l)]);
            eval(['clear mat_1',num2str(l)]);
         ...
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_RCP85_',char(var1),'_',char(period1),'.csv'),rcp85_fin1)
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_RCP85_',char(var1),'_',char(period2),'.csv'),rcp85_fin2)
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_RCP85_',char(var1),'_',char(period3),'.csv'),rcp85_fin3)
          clear rcp85_fin1 rcp85_fin2 rcp85_fin3
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RCMs en MODE HISTORIQUE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
       ...
       mm=length(MODEL_LIST);      
        for mo = 1:mm   %boucle sur les modeles
              ...
              curr_path=PATHM(mo) ;
            % lecture du fichier anomalie
            FichierIn = char ( strcat(main_dir,curr_path,'\',char(var1),'\TENDANCES_SEN_',curr_model,'_',char(var1),'_1971-2000.nc') );  
        ...
        for j=1:(mm)
         ...
         csvwrite(strcat(out_dir,char(domaine),'_Tendances_YEAR_HISTO_',char(var1),'_1971-2000.csv'),histo_fin)
         clear histo_fin
