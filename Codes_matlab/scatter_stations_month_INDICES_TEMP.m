...
out='G:\PROJETS\PROJET_2018\DATA\figures\tendances\MONTH\';
...
path='G:\PROJETS\PROJET_2018\DATA\CANADA\INDICES_MON\Tasmin\TENDANCES';
elseif (strcmp(char(var),'tasmax')==1)
path='G:\PROJETS\PROJET_2018\DATA\CANADA\INDICES_MON\Tasmax\TENDANCES';
elseif (strcmp(char(var),'tasmoy')==1)
path='G:\PROJETS\PROJET_2018\DATA\CANADA\INDICES_MON\Tasmoy\TENDANCES';
end
...
for d=1:4
    tendance=load(strcat(char(path),'\INDICES_MOY_',char(var),'_',char(list_mon(m)),'_',char(domaine(d)),'_',char(per),'_',char(confiance),'.dat'));
...
set(gca,'FontSize',20);
   ...
   if (strcmp(char(list_mon(m)),'01')==1)
     month='January';
   elseif (strcmp(char(list_mon(m)),'02')==1)
     month='February';
   elseif (strcmp(char(list_mon(m)),'03')==1)
     month='March';
   elseif (strcmp(char(list_mon(m)),'04')==1)
     month='April';
   elseif (strcmp(char(list_mon(m)),'05')==1)
     month='May';
   elseif (strcmp(char(list_mon(m)),'06')==1) 
     month='June';
   elseif (strcmp(char(list_mon(m)),'07')==1) 
     month='July';
   elseif (strcmp(char(list_mon(m)),'08')==1)
     month='August';
   elseif (strcmp(char(list_mon(m)),'09')==1)
     month='September';
   elseif (strcmp(char(list_mon(m)),'10')==1)
     month='October';
   elseif (strcmp(char(list_mon(m)),'11')==1)
     month='November';
   elseif (strcmp(char(list_mon(m)),'12')==1)
     month='December';      
   end
 set(gcf,'units','normalized','outerposition',[0 0 1 1]) 
   
if (strcmp(char(var),'tasmax')==1)
          SAVE_file=[strcat(out,'Decadal_trend_of_tasmax_',char(month),'_',char(per),'_',char(confiance))];  
          export_fig(SAVE_file)         
   elseif (strcmp(char(var),'tasmin')==1) 
          SAVE_file=[strcat(out,'Decadal_trend_of_tasmin_',char(month),'_',char(per),'_',char(confiance))]; 
          export_fig(SAVE_file)          
    elseif (strcmp(char(var),'tasmoy')==1) 
          SAVE_file=[strcat(out,'Decadal_trend_of_tasmoy_',char(month),'_',char(per),'_',char(confiance))];  
          export_fig(SAVE_file)        
end
close(gcf);
clear month negatif positif lael1 label2 label3 label4  tendance
end
 clear stat_station
end
end
