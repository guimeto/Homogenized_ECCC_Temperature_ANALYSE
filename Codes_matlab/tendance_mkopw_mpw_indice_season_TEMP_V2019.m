...
%
dd = 'K:\DATA\Donnees_Stations\2nd_generation_V2019\INDICES\INDICES_SEASON\ANOMALY_STD\';
out='K:\DATA\Donnees_Stations\2nd_generation_V2019\INDICES\INDICES_SEASON\TENDANCES_ANO_STD\Tasmax\MOY';
...
for  sea=1:length(seasons) % Boucle mensuelle
for s=1:size(name_stat{1},1)
    
if  (strcmp(curr_var,'Tasmax')==1)
       curr_varin='tasmax'; 
elseif  (strcmp(curr_var,'Tasmin')==1)
       curr_varin='tasmin';
elseif  (strcmp(curr_var,'Tasmoy')==1)
       curr_varin='tasmoy';              
end

File=strcat(char(name_stat{1}{s}),'_ANO_STD_SEASON_',char(curr_varin),'_MOY_',num2str(start_year),'_',num2str(end_year),'_',char(seasons(sea)),'.csv');
filename = strcat(dd,curr_var,'\MOY\',num2str(start_year),'_',num2str(end_year),'\',File); 
...
save(strcat(out,'\INDICES_MOY_',curr_varin,'_',char(seasons(sea)),'_',char(domaine),'_',num2str(start_year),'_',num2str(end_year),'_',char(confiance),'.dat'),'mat_final', '-ASCII');
